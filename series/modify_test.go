...
func TestModify_Insert(t *testing.T) {
	...
	var tests = []struct {
		...
		{"non-default index, empty arg",
			MustNew([]string{"foo", "bar"}, Config{Index: []string{"baz", "qux"}}),
			args{1, "quux", nil},
			want{series: MustNew([]string{"foo", "quux", "bar"}, Config{Index: []string{"baz", "", "qux"}}), err: false}},
		{"multiIndex",
			multi,
