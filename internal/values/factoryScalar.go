...
func ScalarFactory(data interface{}) (Factory, error) {
	...
	case float32, float64:
		...
		f := newFloat64(val)
		ret = Factory{&float64Values{f}, datatypes.Float64}

	case int, int8, int16, int32, int64:
		val := scalarIntToInt64(data)
		i := newInt64(val)
		ret = Factory{&int64Values{i}, datatypes.Int64}
	...
	case string:
		...
		ret = Factory{&stringValues{val}, datatypes.String}

	case bool:
		val := newBool(data.(bool))
		ret = Factory{&boolValues{val}, datatypes.Bool}

	case time.Time:
		val := newDateTime(data.(time.Time))
		ret = Factory{&dateTimeValues{val}, datatypes.DateTime}

	case interface{}:
		val := newInterface(data.(interface{}))
		ret = Factory{&interfaceValues{val}, datatypes.Interface}
